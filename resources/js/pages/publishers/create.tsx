...
import { PublisherProps, PublisherFormData } from '@/types';
...
const PublisherForm: FC<PublisherProps> = ({ publisher }) => {
  ...
  const breadcrumbs = [
    ...
    },
    {
      ...
      href: publisher
        ? route('publishers.edit', { publisher: publisher.id })
  ...
  const sendRequest = () => {
    const targetRoute = publisher
      ? route('publishers.update', { publisher: publisher.id })
    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
        <h1 className="text-center text-[45px]">
          {publisher ? `Update publisher ${publisher.name}` : 'Create publisher'}
        </h1>
